...
const PhoneForm = ({ onCancel, onSubmit, buttonLabel, defaultValues }) => {
    const [inputs, setInputs] = useState({
        ...
        priceNew: { value: defaultValues ? defaultValues.priceNew.toString() : '', isValid: true },
        priceOld: { value: defaultValues ? defaultValues.priceOld.toString() : '', isValid: true },
        quantity: { value: defaultValues ? defaultValues.quantity.toString() : '', isValid: true },
        typeProduct: { value: defaultValues ? defaultValues.typeProduct : '', isValid: true },
        productInfo: { value: defaultValues ? defaultValues.productInfo : '', isValid: true },
    })
    ...
    function submitHandler() {
        ...
        const isValiPriceNew = !isNaN(phonesData.priceNew) && phonesData.priceNew > 0
        const isValiPriceOld = !isNaN(phonesData.priceOld) && phonesData.priceOld > 0
        const isValiQuantity = !isNaN(phonesData.quantity) && phonesData.quantity > 0
        const isValiTypeProduct = phonesData.typeProduct.length > 0
        const isValiProductInfo = phonesData.productInfo.length > 0

        if (!isValiName || !isValiPriceNew || !isValiPriceOld || !isValiQuantity || !isValiTypeProduct || !isValiProductInfo) {
            Alert.alert('Invalid Input', 'Please check your input Value')
            setInputs((currentInput) => {
                return {
                    name: { value: currentInput.name.value, isValid: isValiName },
                    priceNew: { value: currentInput.priceNew.value, isValid: isValiPriceNew },
                    priceOld: { value: currentInput.priceOld.value, isValid: isValiPriceOld },
                    quantity: { value: currentInput.quantity.value, isValid: isValiQuantity },
                    typeProduct: { value: currentInput.typeProduct.value, isValid: isValiTypeProduct },
                    productInfo: { value: currentInput.productInfo.value, isValid: isValiProductInfo },
                }
            })
            return
        }
    ...
    const formIsInvalid = !inputs.name.isValid || !inputs.priceNew.isValid || !inputs.priceOld.isValid || !inputs.quantity.isValid || !inputs.typeProduct.isValid || !inputs.productInfo.isValid
    return (
